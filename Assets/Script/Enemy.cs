...
public class Enemy : MovingObject
{
    ...
    //Destroy the enemy if they and the player oucupy the same spot
    private void Update()
    {
        if(target.position == transform.position)
        {
            //Kill by removing its asset and resting the zombie
    ...
    public void MoveEnemy()
    {
        ...
        if (Mathf.Abs(target.position.x - transform.position.x) < float.Epsilon)
        {
            ...
            yDir = target.position.y > transform.position.y ? .5f : -.5f;
        ...
        {
            yDir = .25f;
            xDir = target.position.x > transform.position.x ? .75f : -.75f;
        }
        else
        {
